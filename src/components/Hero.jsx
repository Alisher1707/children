        ...
        <div className="flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-12">
          ...
          <div className="flex-1 z-10 text-center lg:text-left">
            ...
            <div className="flex flex-col items-center lg:items-start gap-5">
              {/* Button */}
              <button className="font-semibold px-6 sm:px-8 py-2.5 sm:py-3 rounded-full text-sm sm:text-base transition-all shadow-lg" style={{ backgroundColor: '#C6E5F9', color: '#5D4592' }}>
                {t('hero.button')}
              </button>

              {/* Delivery info */}
              <div className="flex items-center gap-3 sm:gap-4 border-2 border-white border-opacity-30 rounded-2xl px-4 sm:px-5 py-2.5 sm:py-3">
                ...
                <div className="text-white">
                  <p className="font-bold text-sm sm:text-base">{t('hero.delivery.title')}</p>
                  <p className="text-xs sm:text-sm opacity-90">{t('hero.delivery.subtitle')}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center relative w-full lg:w-auto">
            <div className="relative scale-75 sm:scale-90 lg:scale-100">
                ...
                {books.map((book, index) => (
                  <img
                    ...
                    className={`absolute transition-all duration-500 ${
                      index === currentSlide
                        ? 'w-44 h-56 sm:w-52 sm:h-64 z-30 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 rotate-0 scale-100'
                        : index === (currentSlide + 1) % books.length
                        ? 'w-36 h-48 sm:w-44 sm:h-56 z-20 left-3/4 top-1/2 -translate-x-1/2 -translate-y-1/2 rotate-12 scale-90 opacity-70'
                        : 'w-36 h-48 sm:w-44 sm:h-56 z-10 left-1/4 top-1/2 -translate-x-1/2 -translate-y-1/2 -rotate-12 scale-90 opacity-70'
                    }`}
                  />
                ))}
              </div>
              ...
              <div className="absolute -bottom-12 left-1/2 -translate-x-1/2 w-[350px] sm:w-[450px] h-[120px]">
            ...
            {/* Navigation arrows */}
            <div className="flex gap-2 sm:gap-3 z-40 mt-8 sm:mt-12">
              <button
                onClick={prevSlide}
                className="bg-white text-purple-700 w-9 h-9 sm:w-11 sm:h-11 rounded-full flex items-center justify-center hover:bg-gray-100 transition-all shadow-lg"
              >
